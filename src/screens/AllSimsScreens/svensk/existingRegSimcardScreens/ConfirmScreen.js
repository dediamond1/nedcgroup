...
import { View, Keyboard, TouchableOpacity, StyleSheet, ScrollView, Alert } from "react-native";
...
export const ConfirmScreen = ({ route, navigation }) => {
    ...
    const { email, street, postalCode, PhoneNumber, city, number, smsCode } = userInfo || {}
    ...
    const bankIdTwoFactor = async (orderRefr) => {
        try {
            ...
            if (
                resData?.response?.userMessage?.length > 0
            ) {
                setLoading(false)
    ...
    const saveInformation = async () => {
        try {
            ...
            if (
                data?.response?.userMessage?.length > 0
            ) {
                setLoading(false)
    ...
    return (
        <>
            {loading || error || success ? <Status success={success} loading={loading} error={error} text={error ? "OBS! något gick fel!" : success ? "SIM-kortet är nu registrerat" : "Registrerar..."} onPressTryAgain={() => startBankId()} onPressCancel={cancel} /> : null}
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <AppScreen style={{ flex: 1 }}>
                    ...
                    <View style={styles.container}>
                        <View>
                            <AppText text={"Personnummer"} style={{ fontSize: 16, color: "#e2027b" }} />
                            <AppText text={PhoneNumber} style={{ fontSize: 16, color: "#000" }} />
                            <AppText text={"MobilNummer"} style={{ fontSize: 16, color: "#e2027b" }} />
                            <AppText text={number} style={{ fontSize: 16, color: "#000" }} />
                            <AppText text={"SMS koden"} style={{ fontSize: 16, color: "#e2027b" }} />
